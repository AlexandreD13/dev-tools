...
def register(subparsers):
    parser = subparsers.add_parser("text_stats", help="Generates statistics on a text.",
                                                         add_help=False,
...
def run(args):
    if args.file:
        ...
        with open(args.file, "r", encoding="utf-8") as f:
            ...
            results["Character frequencies"] = OrderedDict( sorted(Counter(filtered_chars).items()) )
            print(f"{Fore.BLUE}{Style.BRIGHT}" + json.dumps(results, ensure_ascii=False, indent=4) + f"{Style.RESET_ALL}")

            # if args.string: #     print("string")
